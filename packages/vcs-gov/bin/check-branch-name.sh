...
# protected 와 역할 branch 는 이름 규칙의 대상이 아니다. 새로 만드는 branch 가 아니라 이미 있는
# 것이다. 역할 branch 는 선언하지 않았을 수 있으므로 해석된 설정에서 가져온다.
awk -F"$tab" '$1 == "protected" { print $2 }' "$work/branch" >"$work/protected"
if [ -n "${ENGSYS_SYSTEM_ROOT:-}" ]; then
  sh "$(dirname "$0")/branch-settings.sh" settings --project "$project" 2>/dev/null \
    | awk -F"$tab" '$1 == "role" { print $3 }' >>"$work/protected" || true
fi
